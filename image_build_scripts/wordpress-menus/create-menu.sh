...
cd /image_build_scripts/wordpress-menus
...
wp post list --fields=ID,post_title --format=csv > /tmp/posts_along_with_ids.txt
...
for line in `cat /image_build_scripts/wordpress-menus/csv-files/${menu_title}.csv`; do
  ...
  if [[ ${parent_post_title} != 'null' && ${menu_label} == 'null' ]] ; then
    ...
    db_id=`wp menu item list ${menu_title} --fields=db_id,title,object_id | grep "${parent_post_id} *|$" | awk '{print $2}'`
    wp menu item add-post ${menu_title} ${post_id} --parent-id=${db_id}   || exit 1
    set +x
  fi
  ...
  if [[ ${parent_post_title} == 'null' && ${menu_label} != 'null' ]] ; then
    ...
    set -x
    post_id=`grep "${post_title}" /tmp/posts_along_with_ids.txt | awk  -F',' '{print $1}'`
    wp menu item add-post ${menu_title} ${post_id} --title=${menu_label}  || exit 1
    set +x
  fi
  ...
  if [[ ${parent_post_title} != 'null' && ${menu_label} != 'null' ]] ; then
    ...
    parent_post_id=`grep "${parent_post_title}" /tmp/posts_along_with_ids.txt | awk -F"," '{print $1;}'`
    db_id=`wp menu item list ${menu_title} --fields=db_id,title,object_id | grep "${parent_post_id} *|$" | awk '{print $2}'`
    wp menu item add-post ${menu_title} ${post_id} --title=${menu_label} --parent-id=${db_id} || exit 1
    set +x
  fi
